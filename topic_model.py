...
def run_lda(corpus, dictionary, alpha='auto', eta='auto', n_topics='10', num_iter='200', n_words='20', rand=10, passes=10, topic='all', dist=False, workers=None):
    """
    ...
    pases = number to times to read through the whole corpus
        NOTE: chunksize is 10k, so epochs = number of documents / 10k * passes = 560 (with the defaults defined here).
    rand = random_state
    ...
    dist = Boolean for running LdaMulticore.
    workers = number of available cores - 1

    Returns the gensim model object
    """
    ...
    if dist:
        model = ldamulticore.LdaMulticore(corpus=corpus,
                              #alpha=alpha, # can't auto learn alpha in distributed mode
                              eta=eta,
                              num_topics=n_topics,
                              id2word=dictionary,
                              iterations=num_iter,
...
def load_data(corpus_file='path_to_file/file_name', dictionary_file='path_to_file/file_name', model_file='path_to_file/file_name', mmap=None):
    ...
    if model_file == 'path_to_file/file_name':
        ...
        return_objects.append(ldamodel.LdaModel.load(model_file, mmap=mmap))
...
def plot_lda(model, n_topics=100, num_iter=200, n_words=20, topic='all'):
    ...
    for i in range(n_topics):
        if i == 0:
            ...
            arr = np.array(zip([0]*len(mat),mat))
            ax[i].matshow(arr.T, vmin=min_val, vmax=max_val+0.05, cmap=plt.cm.hot)
        ...
        arr = np.array(zip([0]+[0]*len(mat.values),list(mat.values)))
        ax[i+1].matshow(arr.T, vmin=min_val, vmax=max_val, cmap=plt.cm.hot)
    ...
    '''
    file_path = "plots/" + str(topic) + "_LDA_" + str(n_topics) + "topics_" + str(num_iter) + "_iter" + str(n_words) + "_words"+".png"
